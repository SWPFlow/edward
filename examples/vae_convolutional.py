...
def generative_network(z):
  ...
  """
  # TODO normalizer before activations
  with arg_scope([tf.layers.conv2d_transpose],
    ...
    net = tf.layers.conv2d_transpose(net, 32, 5, stride=2)
    net = tf.layers.conv2d_transpose(net, 1, 5, stride=2, activation=None)
...
def inference_network(x):
  ...
  with arg_scope([tf.layers.conv2d, tf.layers.dense],
    ...
    net = tf.layers.conv2d(net, 32, 5, stride=2)
    net = tf.layers.conv2d(net, 64, 5, stride=2)
    net = tf.layers.conv2d(net, 128, 5, padding='VALID')
    net = tf.layers.dropout(net, 0.1)
    net = tf.reshape(net, [M, -1])
    params = tf.layers.dense(net, FLAGS.d * 2, activation=None)

  loc = params[:, :FLAGS.d]
  scale = tf.nn.softplus(params[:, FLAGS.d:])
  return loc, scale
